...
float dt;
float accel = 0, velocity = 0, prev_velocity = 0;
...
int main()
{
    ...
    while (true)
    {
        ...
        float velocity_dot = (velocity - prev_velocity) / dt;
        float yaw_dot = (yaw - prev_yaw) / dt;
        // float angle_dot = (0);
        u[0] = (k[0] * (current_pitch)     // angle
                + k[1] * (-pitch_dot)      // angle over time
                + k[2] * (velocity)        // velocity
                + k[3] * (-velocity_dot)); // velocity over time
